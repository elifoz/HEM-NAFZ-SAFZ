...
filePath = "datasets/NorthAnatolian-1923-2023/" + "combined_NAFZ" + "_timeseries" + ".csv"

dfEq = pd.read_csv(filePath) # dataframe earthquake (dfEq)

dfEq = dfEq[["time", "mag", "sig", "longitude", "latitude", "depth"]]

print(dfEq["time"])
dfEq["time"] = pd.to_datetime(dfEq["time"])
dfEq["timestamps"] = dfEq["time"]
# dfEq.set_index('time', inplace=True)

print(dfEq)

dfEq[dfEq.duplicated('timestamps')] #need to drop repetitive rows (10 rows). There were some overlaps in original data.
print(dfEq)

# Dropping rows
dfEq = dfEq.drop_duplicates(subset=['timestamps'])

dfEq["timestamps"].diff().describe()

dfEq.info()

#FEATURE ENGINEERING
# Number 1: Time intervals between consecutive earthquakes.
dfEq["time_diff"] = dfEq["timestamps"].diff()

dfEq["time_diff_float"] = dfEq["time_diff"].apply(lambda x: x.total_seconds())

#Number 2: the rolling of magnitudes from the last 10 earthquakes

dfEq["mag_roll_10"] = dfEq["mag"].rolling(window=10).mean()

dfEq.dropna(inplace = True)
print(dfEq.describe().T)

dfEq[dfEq["time_diff_float"] > 86400].shape
print(dfEq[dfEq["time_diff_float"] > 86400]) # arasında 1 günden fazla olan deprem sayısı [5342 rows x 10 columns]

min_date = dfEq["time"].min().date()

print(min_date) #en eski tarih 1926-06-29

max_date = dfEq["time"].max().date()
...
df_daily['mag_max'] = dfEq.mag.resample('D').max() # What is the max of events
df_daily['event_count'] = dfEq["mag"].resample('D').count() #How many events happened
df_daily['mag_mean'] = dfEq.mag.resample('D').mean() #What is the mean of values for that particular day
df_daily['mag_sum'] = dfEq["mag"].resample('D').sum() # What is the sum of moments
df_daily['mag_scatter'] = (dfEq.mag.resample('D').std()) # What is the scatter (dağılım) of event magnitudes
df_daily["mag_roll_10"] = df_daily["mag_mean"].rolling(window=10).mean() #son 10 depremin büyüklüklerinin yuvarlanması

df_daily['longitude_mean'] = dfEq.longitude.resample('D').mean() #Mean location of events
df_daily['longitude_std'] = dfEq.longitude.resample('D').std() #Std location of events

df_daily['latitude_mean'] = dfEq.latitude.resample('D').mean() #Mean location of events
df_daily['latitude_std'] = dfEq.latitude.resample('D').std() #Std location of events

df_daily['depth_mean'] = dfEq.depth.resample('D').mean() #Mean location of events
df_daily['depth_std'] = dfEq.depth.resample('D').std() #Std location of events

df_daily['time_diff_float_mean'] = dfEq.time_diff_float.resample('D').mean() #Event spacing
df_daily['time_diff_float_std'] = dfEq.time_diff_float.resample('D').std() #Std location of events

print("aaaaa",df_daily)
file_path = "datasets/NorthAnatolian-1923-2023/df_daily_clean" + ".csv"
...
"""
# INTERPOLATION YÖNTEMİ İLE EKSİK VERİ TAMAMLAMA
...
print(results_df)
"""
#FEATURE ALANLARININ İSMİNİ DEĞİŞTİRME VE ESKİ İSİMLERİ SİLME
...
df_daily_clean_revized.drop(columns = ["mag_scatter", "longitude_std", "latitude_std", "depth_std", "time_diff_float_std", "missing" ], inplace = True) #eksik
file_path = "datasets/NorthAnatolian-1923-2023/df_daily_clean_revized" + ".csv"
...
label = []
cnt = 0
for i, mag in enumerate(df_daily_clean_revized["mag_max"]):
    if (mag>5.5):
        cnt = cnt + 1
...
df_daily_clean_revized["large_eq_label"] = label

print(df_daily_clean_revized.describe().T)

large_eq = df_daily_clean_revized[df_daily_clean_revized["large_eq_label"]>0]
...
df_corr = df_daily_clean_revized.corr()
...
dfEq_plot = df_daily_clean_revized[df_daily_clean_revized["depth_mean"]<50]
plt.hist2d(dfEq_plot['longitude_mean'], dfEq_plot['latitude_mean'], bins=(50, 50), vmax=400)
plt.colorbar()
...
plt.hist2d(dfEq_plot['mag_max'], dfEq_plot['depth_mean'], bins=(50, 50), vmax=400)
plt.colorbar()
plt.xlabel('EQ magnitude')
plt.ylabel('Depth (km)')

# SAN ANDREAS SON 50 YILDA GERÇEKLEŞEN DEPREMLER

dfEq_plot = df_daily_clean_revized.copy()
dfEq_plot["time_days"] = (np.array(dfEq_plot["time_diff_float_mean"])/86_400)
plt.figure(figsize = (14, 10));
dfEq_plot["time_days"].plot.hist(grid=True, bins=100, rwidth=0.9,orientation="horizontal",
                   color='purple')
